...
if(message.channel.permissionsFor(reaw).has('SEND_MESSAGES')) {
  ...
  message.channel.send(reawEmbed.setDescription(` Kanal kilitlendi! Yazma yetkileri alındı.`))
} else {
    permReaw["SEND_MESSAGES"] = null;
  message.channel.createOverwrite(reaw, permReaw);
  message.channel.send(reawEmbed.setDescription(` Kanal kilidi açıldı! Yazma yetkileri geri verildi.`));
};
